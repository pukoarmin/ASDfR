...
public:
    ...
    * Send the supplied double to the port set by the initializer of the class, best used within XDDP.
    * Function is set to MSG_DONTWAIT, therefore if no room in XDDP/IDDP buffer, send will be skipped.
    ...
    * Send array to the supplied port. 
    * Function is set to MSG_DONTWAIT, therefore if no room in XDDP/IDDP buffer, send will be skipped.
    ...
    */
    virtual int sendDoubleArray(double toSend[], int port, int size);
     ...
     */
    virtual int receiveDoubleArray(double *received, int size);
...
{
public:
    IDDPconnection(int _receivePort);
    ~IDDPconnection();
...
{
public:
    XDDPconnection(int _receivePort);
    ~XDDPconnection();
